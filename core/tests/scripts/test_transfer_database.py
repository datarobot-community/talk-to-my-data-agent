...
@pytest.mark.asyncio
async def test_run_requires_different_source_and_target() -> None:
    app_id = "same-app"
...
@pytest.mark.asyncio
async def test_run_requires_apps_stopped_or_paused() -> None:
    source_app_id = "source-id"
    target_app_id = "target-id"
    client = StubClient(
        {
...
@pytest.mark.asyncio
async def test_run_continue_transfer_non_conflicting_uploads_only_new_files() -> None:
    source_app_id = "source-id"
    target_app_id = "target-id"
    client = StubClient(
        {
