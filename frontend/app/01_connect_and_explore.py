...
from core.analyst_db import AnalystDB, DataSourceType, InternalDataSourceType
...
from app.datarobot_connect import DataRobotTokenManager
from app.helpers import state_empty, state_init
...
async def registry_download_callback() -> None:
    ...
    with st.sidebar:  # Use sidebar context
        with st.spinner("Loading selected datasets..."):
            selected_ids = [ds["id"] for ds in selected_registry_datasets]
    ...
    if (
        ...
        with st.sidebar:
            with st.spinner("Loading selected tables..."):
                ...
                if not dataframes:
                    st.error(
                        f"Failed to load data from {os.environ.get('DATABASE_CONNECTION_TYPE')}"
                    )
                    return
        ...
        with st.expander("Database", expanded=False):
            ...
            with st.form("table_selection_form", border=False):
                selected_schema_tables = st.multiselect(
                    label=get_database_loader_message(),
